...
public class PasswordProtect {
	...
	public static String decrypt(String string, String password){
		...
		byte[] code = ParseSystemUtil.parseHexStr2Byte(string);
		// 解密
		byte[] decrypt = PasswordProtect.decryptByAES(code, password);
	 ...
	 * @return 明文
	 */
	private static byte[] decryptByAES(byte[] content, String password) {
		try {
			KeyGenerator kgen = KeyGenerator.getInstance("AES");// 创建AES的Key生产者
